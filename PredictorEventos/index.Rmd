...
#Arreglamos el campo de Circulacion Extracorporea
subsetForPredictor$`CIRCULACION EXTRACORPOREA`[is.na(subsetForPredictor$`CIRCULACION EXTRACORPOREA`)] <- 0 #Asumimos NA = No pump
...
subsetForPredictor$DIABETES <- as.numeric(subsetForPredictor$DIABETES)
...
as.factor(subsetForPredictor$DIABETES)
...
sample.size <- floor(0.75 * nrow(subsetForPredictor))
set.seed(123)
...
hyp.out2 <- glm(`COMPLICACIONES INMEDIATAS`~`CIRCULACION EXTRACORPOREA` + EDAD + DIABETES + `TIEMPO DE BOMBA`,
...
predDat <- with(test.complicaciones,
                expand.grid(age = c(52,33),
                            wkdayr = c(0,10)))


#Tito predictor
Data2 <- Data
str(Data2$`COMPLICACIONES INMEDIATAS`)
levels(Data2$`COMPLICACIONES INMEDIATAS`) 
as.factor(Data2$`COMPLICACIONES INMEDIATAS`)
Data2 <- as.numeric(Data2$`COMPLICACIONES INMEDIATAS`)

hyp.out.Inmediatas <- glm(COMPLICACIONES.INMEDIATAS~EDAD,
               data=Data, family="binomial")  
summary(hyp.out.Inmediatas)
coef(summary(hyp.out.Inmediatas))

hyp.out.Tardias <- glm(COMPLICACIONES.TARDIAS~EDAD,
               data=Data, family="binomial") 
summary(hyp.out.Tardias)
coef(summary(hyp.out.Tardias))

DataTiempoDeBomba <- subset(Data, Data$TIEMPO.DE.BOMBA != "#N/D")
str(DataTiempoDeBomba$TIEMPO.DE.BOMBA)
DataTiempoDeBomba$TIEMPO.DE.BOMBA <- as.numeric(DataTiempoDeBomba$TIEMPO.DE.BOMBA)
sexo <- factor(DataTiempoDeBomba$SEXO)
sexo <- factor(DataTiempoDeBomba$SEXO,labels=c(0,1))
DataTiempoDeBomba$SEXO <- sexo
hyp.out.Complicaciones <- glm(COMPLICACIONES.INMEDIATAS~EDAD+TIEMPO.DE.BOMBA+SEXO,
                              data=DataTiempoDeBomba, family="binomial") 
summary(hyp.out.Complicaciones)
coef(summary(hyp.out.Complicaciones))

pacConCirculacionExtraND <- subset(Data, Data$CIRCULACION.EXTRACORPOREA != "#N/D")
pacConCirculacionExtraND$CIRCULACION.EXTRACORPOREA[pacConCirculacionExtraND$CIRCULACION.EXTRACORPOREA == "NO"] <- 0
pacConCirculacionExtraND$CIRCULACION.EXTRACORPOREA[pacConCirculacionExtraND$CIRCULACION.EXTRACORPOREA != 0] <- 1
sexo <- factor(pacConCirculacionExtraND$SEXO)
sexo <- factor(pacConCirculacionExtraND$SEXO,labels=c(0,1))
pacConCirculacionExtraND$SEXO <- sexo


hyp.out.Complicaciones <- glm(COMPLICACIONES.INMEDIATAS~EDAD+CIRCULACION.EXTRACORPOREA+SEXO,
                              data=pacConCirculacionExtraND, family="binomial") 
summary(hyp.out.Complicaciones)
coef(summary(hyp.out.Complicaciones))
